...
public class DiskManager {
  ...
  public void reconfigureDisks(
    ...
    VirtualDisk vdSystemDisk = getVirtualDisk(configSpec, devices);

    configureSystemDisk(vmConfigSpec, systemDiskMB, vdSystemDisk);
    configureDataDisks(vmConfigSpec, devices, vdSystemDisk);
  }

  protected VirtualDisk getVirtualDisk(
      VirtualMachineConfigInfo configSpec, List<VirtualDevice> devices) throws APPlatformException {
    return getVMSystemDisk(devices, configSpec.getName());
  }

  protected void configureDataDisks(
      VirtualMachineConfigSpec vmConfigSpec, List<VirtualDevice> devices, VirtualDisk vdSystemDisk)
      throws Exception {
    ...
    for (Double dataDiskMB : dataDisksMB) {
      ...
      if (vdKey == 0) {
        ++maxDeviceKey;
        ++maxUnitNumber;
        VirtualDeviceConfigSpec vmDeviceSpec =
            createNewDataDisk(vdSystemDisk, newDiskSpace, maxDeviceKey, maxUnitNumber);
        vmConfigSpec.getDeviceChange().add(vmDeviceSpec);
        paramHandler.setDataDiskKey(vdIndex, maxDeviceKey);
      } else {
        updateDiskConfiguration(vmConfigSpec, devices, vdKey, newDiskSpace);
      }
    }
  }

  protected void updateDiskConfiguration(
    ...
    if (vdDataDisk != null && newDiskSpace > vdDataDisk.getCapacityInKB()) {
      ...
      logger.info(
          "reconfigureDisks() extend data disk #"
              + vdKey
              + " space to "
              + newDiskSpace
              + " ("
      ...
      if (newDiskSpace < vdDataDisk.getCapacityInKB()) {
        logger.error("Cannot reduce size of data disk " + vdDataDisk.getDeviceInfo().getLabel());
        logger.error(
            "Current disk space: "
                + vdDataDisk.getCapacityInKB()
                + " new disk space: "
                + newDiskSpace);
        throw new Exception(Messages.getAll("error_invalid_diskspacereduction").get(0).getText());
      } else if (newDiskSpace > vdDataDisk.getCapacityInKB()) {
        vdDataDisk.setCapacityInKB(newDiskSpace);
        VirtualDeviceConfigSpec vmDeviceSpec = createVirtualDeviceConfigSpec();
        vmDeviceSpec.setOperation(VirtualDeviceConfigSpecOperation.EDIT);
        vmDeviceSpec.setDevice(vdDataDisk);

        vmConfigSpec.getDeviceChange().add(vmDeviceSpec);
      } else {
        logger.debug("Data disk size has not been changed. " + newDiskSpace + " KB");
      }
    }
  }

  protected VirtualDeviceConfigSpec createVirtualDeviceConfigSpec() {
    return new VirtualDeviceConfigSpec();
  }

  protected VirtualDeviceConfigSpec createNewDataDisk(
      VirtualDisk vdSystemDisk, long newDiskSpace, int deviceKey, int unitNumber) throws Exception {

    logger.info("reconfigureDisks() create data disk space with " + newDiskSpace + " KB");

    ManagedObjectReference vmDatastore =
        ((VirtualDeviceFileBackingInfo) vdSystemDisk.getBacking()).getDatastore();
    String vmDatastoreName =
        (String) vmw.getServiceUtil().getDynamicProperty(vmDatastore, "summary.name");

    VirtualDisk vdDataDisk = createVirtualDisk();
    VirtualDiskFlatVer2BackingInfo diskfileBacking = createVirtualDiskFlatVer2BackingInfo();
    ...
    vmDeviceSpec.setFileOperation(VirtualDeviceConfigSpecFileOperation.CREATE);
    vmDeviceSpec.setDevice(vdDataDisk);

    return vmDeviceSpec;
  }

  protected VirtualDiskFlatVer2BackingInfo createVirtualDiskFlatVer2BackingInfo() {
    return new VirtualDiskFlatVer2BackingInfo();
  }

  protected VirtualDisk createVirtualDisk() {
    return new VirtualDisk();
  }

  protected VirtualDisk findDataDisk(List<VirtualDevice> devices, int vdKey) {
  ...
  protected void configureSystemDisk(
      VirtualMachineConfigSpec vmConfigSpec, long systemDiskMB, VirtualDisk vdSystemDisk)
      throws Exception {
    if (systemDiskMB > 0) {
      long newDiskSpace = systemDiskMB * 1024;
      if (newDiskSpace < vdSystemDisk.getCapacityInKB()) {
        logger.error(
            "Cannot reduce size of system disk \""
                + vdSystemDisk.getDeviceInfo().getLabel()
                + "\"");
        logger.error(
            "Current disk size: "
                + vdSystemDisk.getCapacityInKB()
                + " new disk space: "
                + newDiskSpace);
        ...
        logger.info(
            "reconfigureDisks() extend system disk space to "
                + newDiskSpace
                + " ("
                + vdSystemDisk.getDeviceInfo().getLabel()
                + ")");
        ...
        logger.debug("System disk size has not been changed. " + newDiskSpace + " KB");
      }
    } else {
      logger.error(
  ...
  public static long getSystemDiskCapacity(List<VirtualDevice> devices, String vmName)
      throws APPlatformException {
    VirtualDisk vdInfoFirst = getVMSystemDisk(devices, vmName);
    return vdInfoFirst.getCapacityInKB();
  }

  protected static VirtualDisk getVMSystemDisk(List<VirtualDevice> devices, String vmName)
      throws APPlatformException {
    ...
    for (VirtualDevice vdInfo : devices) {
      if (vdInfo instanceof VirtualDisk) {
        ...
        logger.debug(
            "Found system disk in VM "
                + vmName
                + "  label: "
                + vdInfoFirst.getDeviceInfo().getLabel()
                + " type: "
    ...
    if (vdInfoFirst == null) {
      ...
      throw new APPlatformException(
          Messages.getAll("error_missing_systemdisk", new Object[] {vmName}));
    }
