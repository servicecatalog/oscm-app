 ...
 *
 * <p>Please note that almost all signatures of the provisioning service methods contain a user
 * object which represents the calling user. Not all public members of this object are set when the
 * provisionig call is done. Currently, following members are provided by OSCM:<br>
...
@WebService(
    ...
    portName = "ProvisioningServicePort",
...
public class AsynchronousProvisioningProxy implements ProvisioningService {
  ...
  @Inject protected ProvisioningResults provResult;

  @Inject protected AsynchronousProvisioningProxyImpl appImpl;

  @PersistenceContext(name = "persistence/em", unitName = "oscm-app")
  protected EntityManager em;

  @EJB protected ProductProvisioningServiceFactoryBean provisioningFactory;

  @EJB protected APPConfigurationServiceBean configService;

  @EJB protected APPTimerServiceBean timerService;
  ...
  ServiceInstance createPersistentServiceInstance(
    ...
    final HashMap<String, Setting> parameters = createParameterMap(request.getParameterValue());
    String controllerId = parameters.get(InstanceParameter.CONTROLLER_ID).getValue();
    ...
    si.setInstanceAttributes(createAttributes(si, descr.getChangedAttributes()));
  ...
  InstanceDescription getInstanceDescription(InstanceRequest request, User requestingUser)
    ...
    final HashMap<String, Setting> parameters = createParameterMap(request.getParameterValue());
    final HashMap<String, Setting> attributes = createAttributeMap(request.getAttributeValue());
    String controllerId = parameters.get(InstanceParameter.CONTROLLER_ID).getValue();
    ...
    HashMap<String, Setting> customAttributes =
        configService.getCustomAttributes(request.getOrganizationId());
    ...
    settings.setAuthentication(
        configService.getAuthenticationForBESTechnologyManager(controllerId, si));
  ...
  private List<InstanceParameter> createParameters(
      final ServiceInstance si, final Map<String, Setting> src) throws BadResultException {
  ...
  private List<InstanceAttribute> createAttributes(
      final ServiceInstance si, final Map<String, Setting> src) throws BadResultException {
  ...
  public BaseResult updateUsers(String instanceId, List<User> users, User requestingUser) {
    ...
    try {
      ...
      final InstanceStatus status =
          controller.updateUsers(instance.getInstanceId(), settings, mapToServiceUsers(users));
      if (status != null) {
        // forward call to provisioning service on application instance
        if (status.isInstanceProvisioningRequested()) {
          final ProvisioningService provisioning = provisioningFactory.getInstance(instance);
          final BaseResult result = provisioning.updateUsers(instanceId, users, requestingUser);
          if (provResult.isError(result)) {
            return result;
          }
        }
  ...
  public BaseResult deleteUsers(String instanceId, List<User> users, User requestingUser) {
    ...
    try {
      ...
      final InstanceStatus status =
          controller.deleteUsers(instance.getInstanceId(), settings, mapToServiceUsers(users));
      if (status != null) {
        // forward call to provisioning service on application instance
        if (status.isInstanceProvisioningRequested()) {
          final ProvisioningService provisioning = provisioningFactory.getInstance(instance);
          final BaseResult result = provisioning.deleteUsers(instanceId, users, requestingUser);
          if (provResult.isError(result)) {
            return result;
          }
        }
  ...
  public BaseResult asyncModifySubscription(
    ...
    logger.info("Modify parameters for instance {}: {}", instanceId, parameterMap);
    ...
    try {
      ...
      final ProvisioningSettings newSettings =
          new ProvisioningSettings(
              parameterMap,
              attributeMap,
              currentSettings.getCustomAttributes(),
              controllerSettings,
      ...
      if (status != null) {
        ...
        if (status.isInstanceProvisioningRequested()) {
          ...
          final List<ServiceParameter> filteredParameters =
              InstanceFilter.getFilteredInstanceParametersForService(parameterValues);
  ...
  public BaseResult deleteInstance(
    ...
    return appImpl.deleteInstance(instanceId, organizationId, subscriptionId, requestingUser);
  }

  @Override
  public InstanceResult createInstance(InstanceRequest request, User requestingUser) {
    return provResult.getBaseResult(
        InstanceResult.class,
        1,
        Messages.get(getLocale(requestingUser), "error_synchronous_provisioning"));
  }

  private HashMap<String, Setting> createParameterMap(List<ServiceParameter> parameterValues) {
    ...
    if (parameterValues != null) {
      for (final ServiceParameter p : parameterValues) {
        ...
        map.put(p.getParameterId(), new Setting(p.getParameterId(), value, p.isEncrypted()));
      }
    }
    return map;
  }

  private HashMap<String, Setting> createAttributeMap(List<ServiceAttribute> attributeValues) {
    final HashMap<String, Setting> map = new HashMap<>();
    if (attributeValues != null) {
      for (final ServiceAttribute a : attributeValues) {
        ...
        map.put(
            a.getAttributeId(),
            new Setting(a.getAttributeId(), value, a.isEncrypted(), a.getControllerId()));
      }
    }
    return map;
  }
  ...
  public BaseResult activateInstance(String instanceId, User requestingUser) {
    ...
    try {
      ...
      // Forward request
      final InstanceStatus status = controller.activateInstance(instance.getInstanceId(), settings);
      if (status != null) {
        // forward call to provisioning service on application instance
        if (status.isInstanceProvisioningRequested()) {
          final ProvisioningService provisioning = provisioningFactory.getInstance(instance);
          final BaseResult result = provisioning.activateInstance(instanceId, requestingUser);
          if (provResult.isError(result)) {
            return result;
          }
        }
  ...
  void checkInstanceAvailability(ServiceInstance instance)
    ...
    if (!instance.isAvailable()) {
      throw new ServiceInstanceInProcessingException(
          ...
          instance.getInstanceId(), instance.getProvisioningStatus().name());
    }
  }
  ...
  public BaseResult asyncUpgradeSubscription(
    ...
    return modifySubscription(
        ...
        parameterMap,
        attributesMap,
        ProvisioningStatus.WAITING_FOR_SYSTEM_UPGRADE,
        requestingUser);
  }
  ...
  private void mergeServiceUsers(List<User> users, List<ServiceUser> updatedUsers) {
    for (ServiceUser updatedUser : updatedUsers) {
      for (User user : users) {
        if (user.getUserId().equals(updatedUser.getUserId())) {
          user.setApplicationUserId(updatedUser.getApplicationUserId());
          break;
        }
      }
    }
  ...
  public BaseResult saveAttributes(
    ...
    try {
      ...
      logger.warn(e.getMessage(), e);
      return provResult.getErrorResult(BaseResult.class, e, getLocale(requestingUser), null, null);
    }
  }
}
